...
train_dataset.to_excel(model_dir + '1_trainset.xlsx')
test_dataset.to_excel(model_dir + '1_testset.xlsx')
...
def build_model_selu():
  model = keras.Sequential([
    layers.Dense(400, activation='selu', input_shape=[len(normed_train_data.keys())],
                 kernel_initializer='lecun_normal'),
...
def fit_with_model_diff_act(act_fxn, iteration):
  ...
  filename = model_dir + 'model_' + str(iteration + 1) + '.h5'
  if (np.sqrt(k.numpy()) > 20):
      filename = model_dir + 'model_' + str(iteration + 1) + 'notgood' + '.h5'
  model.save(filename)
...
for a in range(0,n_members):
    ...
    print("Session: ", a)
    test_dataset_huber = fit_with_model_diff_act("selu", a)
