...
raw_dataset = pd.read_csv("../Data/latest_tob_simulation_topo_1e6_041920.csv")
dataset = raw_dataset.copy()
dataset.isna().sum()
dataset = dataset.dropna()

test_dataset = dataset.sample(n = 1000)
...
for i in range(n_members):
  ...
  filename = model_dir + 'model_' + str(i + 1) + '.h5'
...
plt.savefig(model_dir + "Model_Average.png")
plt.clf()
...
for i in range(n_members):
  ...
  plt.colorbar()
  _ = plt.plot(lims, lims)
  plt.savefig(model_dir + "Model_" + str(i + 1) + ".png")
  plt.clf()
...
test_under_pred.to_excel(model_dir + 'over_outlier.xlsx')
under_pred_locations.to_excel(model_dir + 'over_out_names.xlsx')
test_labels[(test_labels - yhat) > 30].to_excel(model_dir + 'over_labels.xlsx')
abc = pd.Series(yhat)
abc[(test_labels.values - abc) > 30].to_excel(model_dir + 'over_predictions.xlsx')

#averaged_pred[abs(test_labels.values - yhat) > 30].to_excel(model_dir + 'over_plain_average_out.xlsx')
pred_test_set[abs(test_labels.values - yhat) > 30].to_excel(model_dir + 'over_out_sub_pred.xlsx')
# also filter out those with huge standard deviations
pred_test_set[pred_test_set.T.std() > 30].to_excel(model_dir + 'predict_large_std.xlsx')
test_dataset['ID'] = test_ID
test_dataset['Actual'] = test_labels
test_dataset['Pred_Mean'] = averaged_pred
test_dataset.set_index(np.arange(0,1000))[pred_test_set.T.std() > 30].to_excel(model_dir + 'actual_large_std.xlsx')
test_dataset.set_index(np.arange(0,1000))[abs(test_labels.values - yhat) > 30].to_excel(model_dir + 'just_outlier.xlsx')